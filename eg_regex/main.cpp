...
void regex_replace_test()
{
    ...
    // replace_result = std::regex_replace(text, double_brace, "*");
    // cout << replace_result << "\n";

    // //构造存有结果的字符串，[$&]即为用[]将匹配成功部分括起来
    // // $&	Inserts the matched substring.
    // cout << std::regex_replace(text, double_brace, "[$&]") << '\n';

    // // $i则输出double_brace中第i个括号匹配到的值
    // cout << std::regex_replace(text, double_brace, "$1") << '\n';
    // cout << std::regex_replace(text, double_brace, "[$2]") << '\n';
    // cout << std::regex_replace(text, double_brace, "$3") << '\n';
}
...
int main()
{
    ...
    cout << endl
         << "regex_search examples : " << endl;
    regex_search_test();

    cout << endl
         << "regex_replace examples : " << endl
         << endl;
    regex_replace_test();
